// FILE: components/cases/PostCaseModal.tsx
...
export function PostCaseModal() {
  ...
  return (
    <Modal open={open} onClose={handleClose} ariaLabel="Post a case">
      <div className="sticky top-0 z-[2] flex items-center justify-between border-b border-white/[0.07] bg-bg2 px-7 py-6">
        <h2 className="font-serif text-[26px] font-light tracking-[-0.5px] text-[var(--t)]">
          Post a <em className="italic text-o2">Case</em>
        </h2>
        <button
          type="button"
          onClick={handleClose}
      ...
      >
        ...
        <FormSection title="Case Details">
          <div className="mb-3.5 flex flex-col gap-1.5">
            <label htmlFor="case-title" className="text-xs font-medium text-[rgba(245,240,234,0.55)]">
          ...
          <div className="mb-3.5 flex flex-col gap-1.5">
            <label htmlFor="case-desc" className="text-xs font-medium text-[rgba(245,240,234,0.55)]">
          ...
          <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            <InputField id="incident-date" label="Date of Incident" required error={errors.incidentDate?.message} type="date" register={register('incidentDate')} />
            <InputField id="deadline" label="Application Deadline" required error={errors.deadline?.message} type="date" register={register('deadline')} />
          </div>
        </FormSection>

        <FormSection title="Opposing Party">
          <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            <InputField id="opposing-name" label="Opposing Party Name" required error={errors.opposingName?.message} register={register('opposingName')} />
            <SelectField
              id="opposing-rel"
              label="Relationship"
          ...
          <InputField id="location" label="Location (City, State)" required error={errors.location?.message} register={register('location')} />
        </FormSection>

        <FormSection title="Your Contact Information">
          <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            <InputField id="contact-name" label="Full Name" required error={errors.contactName?.message} register={register('contactName')} />
            <InputField id="contact-email" label="Email" required error={errors.contactEmail?.message} type="email" register={register('contactEmail')} />
          </div>
          <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            <InputField id="contact-phone" label="Phone (optional)" error={errors.contactPhone?.message} register={register('contactPhone')} />
        ...
        >
          ...
          <span className="text-[12.5px] leading-relaxed text-[var(--tm)]">
            I agree to the{' '}
            <Link href="#" className="text-o2 no-underline hover:underline">
              Terms &amp; Conditions
            </Link>{' '}
